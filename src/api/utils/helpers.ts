...
import { BCRYPT_SALT_ROUNDS, JWT_KEY, JWT_EXPIRES_AT } from '../../config';
import { ICharge, IStation, IUser } from '../interfaces';
...
/**
 * @function signStationToken
 * @param user user object
 * @returns {string}
 */
export const signStationToken = ({ _id, email, code }: Partial<IStation>): string => {
  return jwt.sign({ _id, email, code }, JWT_KEY as string, { expiresIn: JWT_EXPIRES_AT });
};
...
/**
 * @function chargePushNotification
 * @param status charge status
 * @param token user pushToken
 * @param _id charge Id
 */
export const chargePushNotification = async (status: ICharge['status'], token: string, _id: Types.ObjectId) => {
  switch (status) {
    case 'LOCKED':
      await pushNotification.chargeLocked(token, _id);
      break;
    case 'UNLOCKED':
      await pushNotification.chargeUnlocked(token, _id);
      break;
    case 'CANCELLED':
      await pushNotification.chargeCancelled(token, _id);
      break;
    case 'CHARGING':
      await pushNotification.chargeStarted(token, _id);
      break;
    case 'COMPLETED':
      await pushNotification.chargeCompleted(token, _id);
      break;
